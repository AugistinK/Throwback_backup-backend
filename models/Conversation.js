...
conversationSchema.statics.createGroup = async function(creatorId, name, participants, description) {
...
conversationSchema.methods.removeParticipant = async function(userId) {
  ...
  this.groupAdmins = this.groupAdmins.filter(a => a.toString() !== userId.toString());
  this.modified_date = Date.now();
  await this.save();
  return this;
};
